...
2. BNK-03 の Knowledge Retrieval ノード（「口座ナレッジ検索」）で `account-kb-placeholder` を差し替え
3. **LLM ノードのコンテキスト設定**（必須）:
   - 「口座対応LLM」ノードを開き、プロンプトエディタで `/` → コンテキストブロック（`#context#`）を挿入
   - KR ノード（「口座ナレッジ検索」）の出力を参照していることを確認
